...
void CClient::m_InitSocket()
{
    ...
    {
        m_pDlg->m_FileLog("HSChat_Log.txt", "WSAStartup Error ");
    }

    if ((m_socket = socket(PF_INET, SOCK_STREAM, 0)) == INVALID_SOCKET)
    {
        m_pDlg->m_FileLog("HSChat_Log.txt", "socket Error ");
    }
    memset(&m_addr, 0, sizeof(m_addr));
...
{    
    ...
    {
        m_pDlg->m_FileLog("HSChat_Log.txt", "SSL_write(Head) Error ");
    ...
    else
    {
        ...
        {
            m_pDlg->m_FileLog("HSChat_Log.txt", "SSL_write(Body) Error ");
            AfxMessageBox(_T("서버에 연결할 수 없습니다."));
        }
    }
    m_InitData();
